...
from roar_net_api.algorithms import best_improvement
...
def test_uniform_tie_breaking(n: int, moves_order: MovesOrder) -> None:
    """
    Tests that the best_improvement method selects the best solution
    uniformly when doing tie breaking.

    Builds a problem that returns n equally optimal moves (and another
    n non-optimal) then runs best improvement m = n * r times. We
    expect best improvement to select optimal moves with a frequency
    of about r for each each optimal move and exactly 0 non-optimal
    moves.
    ...
    @dataclass
    @final
    class Move(SupportsApplyMove[Solution], SupportsObjectiveValueIncrement[Solution]):
        i: int
        opt: bool
    ...
    @final
    class Problem(
        SupportsLocalNeighbourhood[Neighbourhood],
    ):
    ...
    for _ in range(trials):
        ...
        prob = Problem()
        for _ in range(m):
            s = best_improvement(prob, Solution())
